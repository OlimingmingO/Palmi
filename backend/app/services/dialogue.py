...
_PROMPT_FILE = "/data/prompts/system_prompt.txt"


def _load_system_prompt_template() -> str:
    """Load system prompt from file (hot-reload). Falls back to hardcoded default."""
    try:
    ...
    return XIAO_BAN_SYSTEM_PROMPT


def _build_system_prompt(memory_context: str = "", profile_context: str = "") -> str:
    """Build the system prompt with current date, profile, and memory context."""
    now = datetime.now()
    ...
    if profile_context.strip():
        profile_section = f"关于这位老人的背景信息：\n{profile_context}"

    memory_section = ""
    if memory_context.strip():
        memory_section = f"你记得关于这位老人的近期相关信息：\n{memory_context}"
...
async def get_reply(
    ...
    system_prompt = _build_system_prompt(memory_context, profile_context)
...
async def generate_greeting(elder_id: str, memory_context: str = "", profile_context: str = "") -> str:
